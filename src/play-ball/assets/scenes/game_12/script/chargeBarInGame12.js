...
cc.Class({
    ...
    onLoad () {
        ...
        this.ballScript = this.player.getComponent('ballInGame12');
        ...
        this.Canvas.on(cc.Node.EventType.TOUCH_START, e => {
            if(this.ballRigidBody.linearVelocity.mag() < 50 && com.mouse === 1) {
        ...
        this.Canvas.on(cc.Node.EventType.TOUCH_END, e => {
            if(this.ballRigidBody.linearVelocity.mag() < 50 && com.mouse === 1 && this.border.opacity != 0) {
                this.border.opacity = 0;
                this.launch();
            }
        })
